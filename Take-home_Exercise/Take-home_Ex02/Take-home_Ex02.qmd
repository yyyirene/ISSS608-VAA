...
knitr::kable(head(edges_1st_full_named, 6))
...
knitr::kable(head(edges_selected, 6))
...
knitr::kable(head(selected_types, 6))
```
...
knitr::kable(head(edges_1st_filtered, 6))
```
...
```{r}
#| code-fold: true
#| code-summary: "What we can find from the network？"
...
knitr::kable(head(edges_others_full, 6))
```
...
knitr::kable(head(edges_2nd_full, 6))
```
...
knitr::kable(head(edges_people_to_2nd_tbl, 10))
```
...
```{r}
knitr::kable(head(influenced1, 10))
```

...
pacman::p_load(tidyverse, jsonlite,
               tidygraph, ggraph,ggplot2,SmartEDA,igraph,visNetwork)
```
...
![](images/clipboard-2426498860.png){width="266"}
...
Firstly, I created an `extract_subnetwork` function to simplify the process of extracting network data. The methodology was referenced from a [senior's work.](https://isss608-kjcpaas.netlify.app/take-home_exs/ex3/take-home_ex3#mini-challenge-3) In this case, I also added a `direction` parameter to allow for further filtering.
...
1.  Who has she collaborated with and directly or indirectly influenced?
...
digraph sailor_vertical {
  ...
  # 左列：问题（竖排）
  Q1 [label = 'Q1:\\nInfluenced by others', shape = diamond]
  Q2 [label = 'Q2:\\nCollaboration & Influence', shape = diamond]
  Q3 [label = 'Q3:\\nInfluence on Oceanus Folk', shape = diamond]

  # 右列：回答（每个问题对应多个，竖着排，不强制 rank）
  Q1a [label = 'Person → Sailor Shift → Work']
  Q1b [label = 'Work → Sailor Shift → Work']

  Q2a [label = 'Sailor Shift → Person → Work\\n(direct collaboration)']
  Q2b [label = 'Sailor Shift → Work → Person\\n(indirect collaboration)']
  Q2c [label = 'Sailor Shift → Person → Work\\n(direct influence)']
  Q2d [label = 'Sailor Shift → Work → Person\\n(indirect influence)']

  Q3a [label = 'Sailor Shift → Person/Community → Work\\n(Oceanus Folk)']
...
Second we will first answer ：How has she influenced collaborators of the broader Oceanus Folk community?
...
edges_to_sailor_full <- edges_to_sailor %>%
  ...
  select(`From Name`, `From Type`, `To Name`, `To Type`, `Edge Type`)

```

```{r eval=FALSE }
edges_to_sailor_full
```
...
All edges starting from Sailor Shift (i.e. first layer edges)
...
Add type information of the from node
...
edges_1st_full_named
```

Below is all the information on the second layer, and remove if there is no other layer
...
And we have select the node that have connect the the third layer and to see the indirectly being influenced.
...
And next we are going to add the time dimension with the influnce.
...
edges_people_to_2nd_tbl
```

Here are the final plot that she has been influenced directly and indirectly.
